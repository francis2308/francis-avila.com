...
function changeBg(id) {
    document.querySelectorAll('.section-5 .section-bg').forEach(function(bg) {
        bg.style.opacity = '0';
    });
...
function handleDevice() {
    ...
    document.querySelectorAll('.project-box').forEach(function(box) {
        const bgId = box.getAttribute('data-bg');

        if (isMobile) {
            box.onclick = function() {
                changeBg(bgId);
            };
            box.onmouseover = null; // Deshabilita el cambio por hover en móvil
        } else {
            box.onclick = null; // Deshabilita el cambio por clic en escritorio
